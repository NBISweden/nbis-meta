...
def get_kegg_ortholog_info(outdir, tmpdir="$TMPDIR"):
    ...
    with open(ko_out, "w") as fh_kos, open(ko2path_out, "w") as fh_ko2path, open(
        ko2ec_out, "w"
    ) as fh_ko2ec:
        ...
        for ko_id, l in hier.items():
            for i, d in enumerate(l):
                ...
                pathways[d["pathway"]] = {
                    "Pathway_category1": d["KO_category1"],
                    "Pathway_category2": d["KO_category2"],
                }
                for enzyme in d["enzymes"]:
                    try:
                        ec2path[enzyme].append(
                            "map{}".format(d["pathway"].split(" ")[0])
                        )
                    except KeyError:
    ...
    with open(pathways_out, "w") as fh_pathways:
        fh_pathways.write(
            "{}\t{}\t{}\t{}\n".format(
                "Pathway_id", "Pathway_name", "Pathway_category1", "Pathway_category2"
            )
        )
...
def get_kegg_module_info(outdir, tmpdir="$TMPDIR"):
    ...
    with open(modules_out, "w") as fh_modules:
        fh_modules.write(
            "Module_id\tModule_name\tModule_category1\tModule_category2\tModule_category3\n"
        )
...
def parse_ko_annotations(annotations, dldir, outdir, map_go=False):
    ...
    annot = pd.read_csv(
        annotations,
        header=None,
        usecols=[0, 2, 3, 6, 8, 9, 10, 14, 15, 21],
        names=[
            "orf",
            "evalue",
            "score",
    ...
    logging.info(
        "{} ORFs with {} KOs".format(
            len(orf2ko["orf"].unique()), len(orf2ko["ko"].unique())
        )
    )
    logging.info("Mapping ORFs to modules")
    orf2module = feature2orf(annot.loc[annot["module"] == annot["module"]], "module")
    logging.info(
        "{} ORFs in {} modules".format(
            len(orf2module["orf"].unique()), len(orf2module["module"].unique())
        )
    )
    logging.info("Mapping ORFs to pathways")
    orf2pathway = feature2orf(
        annot.loc[annot["pathway"] == annot["pathway"]], "pathway", match="map"
    )
    logging.info(
        "{} ORFs in {} Pathways".format(
            len(orf2pathway["orf"].unique()), len(orf2pathway["pathway"].unique())
        )
    )
    logging.info("Mapping ORFs to transporters")
    orf2tc = feature2orf(annot.loc[annot["tc"] == annot["tc"]], feature="tc")
    logging.info(
        "{} ORFs in {} transporters".format(
            len(orf2tc["orf"].unique()), len(orf2tc["tc"].unique())
        )
    )
    ...
    logging.info(
        "{} ORFs in {} CAZYs".format(
            len(orf2caz["orf"].unique()), len(orf2caz["cazy"].unique())
        )
    )
    ...
    ko2ec = pd.read_table(
        "{}/kegg_ko2ec.tsv".format(dldir), header=None, names=["ko", "ec"], index_col=0
    )
    ...
    # Add pathway info
    orf2pathway = pd.merge(
        orf2pathway, pathways, left_on="pathway", right_index=True, how="left"
    )
    orf2pathway.set_index("orf", inplace=True)
    orf2pathway.sort_index(inplace=True)
    # Add module info
    orf2module = pd.merge(
        orf2module, modules, left_on="module", right_index=True, how="left"
    )
    ...
    if map_go:
        ...
        logging.info(
            "{} ORFs in {} GO terms".format(
                len(orf2go["orf"].unique()), len(orf2go["GO"].unique())
            )
        )
    # Write files
...
def main():
    ...
    download_parser.add_argument(
        "dldir", help="Write files to this directory. Will be created if missing."
    )
    download_parser.set_defaults(func=download)
    # Annotation parser
    annot_parser = subparser.add_parser("parse", help="Parse emapper annotation files")
    annot_parser.add_argument(
        "dldir", help="Directory used to store KEGG info files (from download command)"
    )
    annot_parser.add_argument(
        "annotations",
        help="emapper.py annotation files (Typically *.emapper.annotations)",
    )
    annot_parser.add_argument("outdir", help="Output directory for parsed files")
    annot_parser.add_argument(
        "--map_go", action="store_true", help="Also map GO terms (can take a long time)"
    )
    annot_parser.set_defaults(func=parse)
    # Sum annotations parser
    quant_parser = subparser.add_parser(
        "quantify", help="Add abundances and summarize KEGG features"
    )
    quant_parser.add_argument("abundance", help="Abundance table")
    quant_parser.add_argument("parsed", help="Parsed file (from parse command)")
    quant_parser.add_argument(
        "outfile", help="Output file with summed abundances for features"
    )
    quant_parser.add_argument(
        "--normalize",
    ...
    merge_parser = subparser.add_parser(
        "merge", help="Merge two or more files from quantify step"
    )
    merge_parser.add_argument("files", nargs="+", help="Files from quantify step")
    merge_parser.add_argument("outfile", type=str, help="Output file")
