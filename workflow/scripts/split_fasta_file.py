...
def count_seqs(f):
    ...
    """
    p = run(["grep", "-c", ">", f], capture_output=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--n_files", type=int, default=10, help="Number of files to split into"
    )
    parser.add_argument(
        "-N", "--n_seqs", type=int, help="Number of sequences in input file"
    )
    parser.add_argument(
        "-o", "--outdir", type=str, default=".", help="Output directory for split files"
    )
    parser.add_argument(
        "--prefix", type=str, default="split", help="Prefix for split files"
    )
