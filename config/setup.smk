...
if 'uppmax.uu.se' in hostname:
    ...
    with open("cluster.yaml", 'r') as stream:
        try:
            clusterSpec = yaml.load(stream)
...
if config["trimmomatic"]:
    ...
    for key in ["adapter_sequence", "rev_adapter_sequence"]:
        config_params.append(("    - ","{}: {}".format(key,config["cutadapt"][key])))
else:
...
if(os.path.isfile(config["sample_list"])):
    ...
    if len(assemblyGroups) > 0 and config["assembly"]:
        ...
        if config["binning"]:
            ...
            config_params.append(("   - Map dir",os.path.abspath(config["map_dir"])))
            config_params.append(("   - Bin dir",os.path.abspath(config["bin_dir"])))
            if config["checkm"]:
...
rule write_config:
    ...
    run:
        ...
        with open(output[0], 'w') as fh_out:
            fh_out.write("{}\n{}\n".format("Pipeline configuration","-"*len("Pipeline configuration")))
